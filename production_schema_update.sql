...
-- 3. Migrate existing department assignments to new table
-- This ensures existing users maintain their department assignments
INSERT INTO "userDepartments" ("userId", "departmentId", "isPrimary", "assignedAt")
SELECT 
    "id" as "userId",
    "departmentId",
    TRUE as "isPrimary",  -- Mark existing assignments as primary
    NOW() as "assignedAt"
FROM "users" 
...
-- 8. Add indexes for better task filtering performance
...
-- 18. Create channels table for messaging
...
-- 19. Create channel members table
...
-- 22. Add necessary indexes for performance
...
-- 2. Run this script: psql -d your_database_name -f production_schema_update.sql
