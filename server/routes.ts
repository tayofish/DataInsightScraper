  ...
  wss.on('connection', (ws: ExtendedWebSocket, req) => {
    ...
    ws.on('message', async (message: string) => {
      try {
        ...
        // Handle edit channel message
        if (data.type === 'edit_channel_message' && ws.userId) {
          try {
            ...
            if (!messageId || !channelId || !content) {
              ws.send(JSON.stringify({
                type: 'error',
                message: 'Missing required fields for editing message'
              }));
              return;
            }
            ...
            if (existingMessage.userId !== ws.userId) {
              ws.send(JSON.stringify({
                type: 'error',
            ...
            // Get the updated message with user information
            const updatedMessage = await db.query.messages.findFirst({
              where: (fields, { eq }) => eq(fields.id, messageId),
              with: {
                user: true
            ...
            console.log('Broadcasting edited message to channel members');
            wss.clients.forEach((client: ExtendedWebSocket) => {
              if (client.readyState === WebSocket.OPEN) {
                client.send(JSON.stringify({
                  type: 'channel_message_updated',
            ...
            console.error('Error handling edit_channel_message:', error);
            ws.send(JSON.stringify({
              type: 'error',
